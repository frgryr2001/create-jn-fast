...
const defaultTargetDir = 'my-jn-project';
...
async function main() {
  ...
  try {
    ...
    if (p.isCancel(extraToolsResponse)) {
      p.cancel('Operation cancelled.');
      process.exit(1);
    }

    results.extraTools = extraToolsResponse as string[];
  } catch (cancelled: any) {
  ...
  const { template, overwrite, packageName, extraTools: selectedTools } =
    results;
    ...
    for (const toolValue of selectedTools) {
      ...
      if (tool?.action) {
        try {
          ...
          p.log.success(`✓ ${tool.label} configured`);
        } catch (error) {
          p.log.warn(
            `⚠ Failed to configure ${tool.label}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    }
